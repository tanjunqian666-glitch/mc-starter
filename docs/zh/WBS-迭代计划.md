...
> **项目状态**：P1 ✅ | P2 ✅ | P3 ✅ | P0x ✅ | P6 ✅ | P5 全部完成 ✅ | Sprint 10 ✅ | **GUI v1 交付 ✅ 2026-05-02**
>
> **下一阶段**：恢复 stash → 修复 3 个 BUG → v1.1 布局优化 → v1.2 修 BUG → v2（修复入口+崩溃检测+备份恢复）
>
> **v2 代码已归档**：修复窗口 GUI、orchestrator 修复函数、崩溃守护进程已移入 `_v2/` 目录，不参与当前构建。v2 启动时恢复。
...
| 单元测试 | 151 (Linux) + 9 (GUI Windows) = 160 全通过 ✅ |
| CLI 构建 | `starter.exe` 11MB ✅ |
| GUI 构建 | `starter-gui.exe` 11MB (`-H windowsgui`, CGO) ✅ |
| GUI 启动 | ✅ 成功显示窗口，无崩溃 |
| 控件完整性 | ⚙ 设置按钮 ✅ · 版本下拉框 ✅ · 安装/更新/已最新 ✅ |
| | 📂 打开启动器 ✅ · 版本信息栏 ✅ · 状态文字 ✅ |
| | 最小化/最大化控制 ✅（最大化禁用） |
| 按钮状态 | 已最新时禁用 ✅ |
| 设置弹窗功能 | debug log 确认 Dialog.Create + dlg.Run() 正常 ✅ |
| 设置弹窗内容 | API 地址/启动器路径/MC 目录/保存/取消/副版本 ✅ |
| 窗口 DPI 缩放 | 逻辑尺寸 380×230，物理尺寸 570×345（150% DPI） |
| WinAppDriver 截图 | ❌ 已知限制（远程 session 不支持） |

### v1 已知问题（暂不阻塞发布）

| 问题 | 说明 | 计划 |
|------|------|------|
| 🔧 修复入口 | 主窗口无修复按钮，相关代码已就绪但暂不暴露 | v2 激活 |
| 进度条撑大窗口 | walk `SetVisible` 后 layout 不重算 | v2 修 |
| 安装按钮可能无响应 | `CanSync()` 与 `packStatusLocked()` 检测不一致 | v2 修 |
| 安装完成不刷新 UI | `EvtSyncDone` 处理顺序问题 | v2 修 |
| WinAppDriver click ⚙ 报 500 | 远程 session 对 walk 模态对话框兼容性有限 | 手动测试，代码无问题 |
...
- `SetVisible` 切换后 walk layout 可能不会自动重新计算窗口大小（进度条撑大问题）
- walk `ComboBox` 不支持单项禁用（版本下拉无法灰色显示未启用版本）
...
- ✅ 主窗口：版本下拉、设置按钮、安装/更新/已最新、打开启动器
- ✅ 设置弹窗：API 地址、启动器路径、MC 目录、副版本管理
- ✅ 事件总线 + 状态机 + ViewModel + Orchestrator 分层架构
- ✅ 更新流程对接真实代码（UpdatePack + EnsureVersion）
- ✅ 错误处理 + 取消同步
- ✅ 窗口固定大小 + 最大化禁用

### v1.1（stash 恢复 + BUG 修复，下一阶段）

> 以下改动已在 `git stash` 中（3 文件 258 行改动），`git stash pop` 后调试。

| 任务 | 内容 | 涉及文件 |
|------|------|---------|
| 中文命名 | 窗口标题 → "Minecraft 版本管理工具"，托盘提示同步中文 | `app.go`, `tray_windows.go` |
| 布局优化 | 标题栏去重、操作按钮居中、进度行与状态行合并，窗口高度 230→190 | `app.go` |
| 刷新按钮 | 下拉框旁加 🔄，点击刷新服务端列表 + 重新评估状态，5s 防抖 | `app.go`, `viewmodel.go` |
| 本地文件检测 | `updatePackStateLocked()` 检测 `packs/{packName}/.mcpack` 存在性 | `viewmodel.go` |

### v1.2（BUG 修复）

| BUG | 根因 | 修复方式 |
|-----|------|---------|
| **安装按钮无响应** | `CanSync()` 和 `packStatusLocked()` 的本地文件检测状态不一致 | 统一检测逻辑 |
| **进度条撑大窗口** | walk layout 在 `SetVisible` 切换后不会自动重算窗口大小 | `SetVisible` 后显式调 `Layout.Update()` |
| **安装完成不刷新 UI** | `EvtSyncDone` 处理中应先 `RefreshState()` 再 `refreshUI()` | 调整事件处理顺序 |

### v2（计划）

- 🔧 修复工具窗口（主窗口添加按钮 + 全量/MC本体/模组同步）
- 🛡️ 崩溃检测 + 静默守护
- 💾 备份恢复 GUI（修复窗口中的备份下拉框 + 恢复按钮）
- 📤 崩溃日志上传
- 代码签名（自签名 / Inno Setup）

### 12.6 代码签名（待决策，不限 v1）
- 小范围分发：自签名证书 + Inno Setup 打包（缓解 SmartScreen）
- 正式发布：Azure Trusted Signing（需海外信用卡）或 winget 上架
- 当前建议：直接用 GitHub Releases + 信誉积累
