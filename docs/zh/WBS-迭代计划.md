...
## 十二、GUI 后续优化（2026-05-02 讨论，待实施）

> 以下改动已 `git stash`，需要 `git stash pop` 后继续调试。

### 12.1 产品命名
- 中文名：**Minecraft 版本管理工具**（窗口标题栏显示）
- 英文仓库名：`mc-starter` 保留不变
- 托盘提示文字同步改为中文

### 12.2 GUI 布局调整
```
┌─────────────────────────────────────────┐
│  版本: [下拉框(180px)         ] [🔄][⚙] │  ← 三元素居中，下方操作按钮对称
├─────────────────────────────────────────┤
│        [📂 打开启动器] [📥 安装]       │  ← 居中
├─────────────────────────────────────────┤
│  [▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬] 66%           取消]  │  ← 进度/状态复用同一行
...
└─────────────────────────────────────────┘
```
- 窗口高度从 230px 缩至 190px
- 进度行和状态行合并，复用同一行空间
- 标题只显示在窗口栏，内容区不重复

### 12.3 刷新按钮
- 点击后重新拉取服务端列表 + 重新评估本地状态
- 5 秒防抖（成功后才防抖，失败随时重试）
- 视觉反馈：按钮闪一下表示安慰剂，失败时状态栏红色提示

### 12.4 本地文件同步检测
- `updatePackStateLocked()` 检测 `packs/{packName}` 目录是否含 `.mcpack`
- 用户手动删除文件后 → 自动识别为"未安装"

### 12.5 遗留 BUG（需解决）
- **安装按钮无响应**：`CanSync()` 和 `packStatusLocked()` 的本地文件检测状态不一致，配置记录版本号清空后 UI 仍显示安装按钮但点击无反应
- **进度条撑大窗口**：walk layout 在 `SetVisible` 切换后不会自动重新计算窗口大小，进度行出现时窗口变宽/变高，消失后不能恢复
- **安装完成不刷新 UI**：`EvtSyncDone` 处理中需要先调用 `RefreshState()` 再 `refreshUI()`
