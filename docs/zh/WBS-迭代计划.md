...
P2 编码前：修复与备份 → GUI 界面 → 崩溃监控
   │  ├─ 新增: pcl_detect.go (PCL2 检测器)
   │  └─ 新增: finder.go (版本目录查找, PCL 优先+回退)
   │
   ▼
...
>
> **代码实现注释**：部分功能模块没有独立文档，对应实现代码为 TODO 标记的入口。如 TUI 界面在 `internal/tui/`，版本查找器在 `internal/launcher/finder.go`。WBS 中标记 `(code: xxx)` 表示该条目有直接可用的代码实现。
