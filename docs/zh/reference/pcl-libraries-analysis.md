...
## 四、PCL2 整合包安装的核心机制：McInstallLoader

### 1. 核心公式

PCL2 安装**任何**整合包（CurseForge / Modrinth / HMCL / 普通 zip）都遵循同一模式：

```
从包格式解析 → (minecraftVersion, loaderType, loaderVersion, 文件列表)
         ↓
McInstallLoader(Request)  ← 统一安装引擎，不关心来源
         ↓
versions/<instanceName>/   ← 一个完整的、可启动的版本
  ├── <instanceName>.json      (version.json，含 libraries + 继承链)
  ├── <instanceName>.jar       (client.jar)
  ├── mods/                    (来自 overrides + 额外下载)
  ├── config/                  (来自 overrides)
  ├── resourcepacks/
  └── PCL/                     (个性化配置)
```

### 2. McInstallRequest 结构

```vb
Dim Request As New McInstallRequest With {
    .NewInstanceName = InstanceName,           ← 版本名（如 "我的整合包1.0"）
    .VersionFolder = "...versions\{name}\",    ← 目标目录
    .MinecraftName = "1.21.1",                 ← MC 版本
    .ForgeVersion = "47.3.0",                  ← Forge 版本（空=不装）
    .NeoForgeVersion = Nothing,                ← NeoForge 版本
    .FabricVersion = "0.16.10"                 ← Fabric 版本（空=不装）
}
```

### 3. McInstallLoader 内部流程

```
McInstallLoader(Request) →
  1. 下载 MC client.jar（Mojang/BMCLAPI）→ versions/{name}/{name}.jar
  2. 下载 MC libraries（解析 version.json 继承链）
  3. 如果 loader != ""：
     a. 下载 loader 的 installer/processor jar
     b. 运行 loader 安装（生成 version.json、下载额外 libraries）
     c. 写入 versions/{name}/{name}.json（合并后的完整版本描述）
  4. 返回完成
```

**关键设计**：`McInstallLoader` 不关心来源，只关心 `(minecraftVersion, loaderType, loaderVersion)`。

### 4. 对我们的启示

| PCL2 | 我们的对应 |
|------|-----------|
| 解包 zip → 读 manifest.json → `(mc, loader, mods列表)` | **服务端** import zip → `(mc_version, loader, file manifest)` |
| `McInstallLoader` — 本地下 MC + Loader | **已有** `sync`（下 MC 本体）+ `fabric install`（下 Loader），但没串起来 |
| 解压 overrides/ → versions/ | **已有** `update` → 下载到 packs/ |
| 最终：`versions/<name>/` 完整版本 | **缺失**：packs/ 与 versions/ 的整合逻辑 |

**结论**：缺一个 `EnsureVersion(ctx)` 级别的编排函数，把 `sync`、`fabric/forge install`、`update` 串成一条流水线。

---

## 五、从 PCL 源码中确认的一些实现细节
